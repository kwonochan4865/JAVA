...
//	VideoExample ve = new VideoExample(); -> 리스트 배열을 생성하지 않고 백터를 이용
// ve.getvecEnroll();	
	List<Video> infolist = new ArrayList<Video>();
	while((line= br.readLine()) !=null) { 
		String[] writeStr = line.split("∞"); // write가 리모콘인 배열을 만들어 ∞를 기준으로 잘라 담음
		
		if(writeStr.length !=4) continue; // ∞를 기준으로 자른 내용의 길이가 3개가 아니라면 계속(즉, 3개를 자르라는 말)
		...
		personinfo.setOrder(Integer.parseInt(writeStr[2])); // 자른 배열 중 2번 인덱스를 personinfo.setInt에 담음
		personinfo.setOrder(Integer.parseInt(writeStr[3])); // 자른 배열 중 3번 인덱스를 personinfo.setInt에 담음
